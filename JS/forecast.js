...
class WeatherForecast{
    constructor() {}
        ...
        async fetchAsync (whatCity) {
            //current forecast
            URL=`http://api.openweathermap.org/data/2.5/weather?units=metric&q=${cityName}&appid=${key}`;
        ...
        async fetchForecastAsync (whatCity) {
            //5 days forecast
            URL=`http://api.openweathermap.org/data/2.5/forecast?units=metric&q=${cityName}&appid=${key}`;
            // await response of fetch call
            let response = await fetch(URL);
            // only proceed once promise is resolved
            let data = await response.json();
